...
/**
 * Compares billed unit prices against the curated reference dataset for
 * non-medicine items (procedures/tests/consumables/charges). Medicines are
 * handled by savings.ts instead, which carries the doctor/pharmacist
 * disclaimer required for substitution-flavored findings.
 */
  ...
  for (const item of items) {
    ...
    if (item.unit_price > reference.reference_price * OVERAGE_THRESHOLD) {
      findings.push({
        ...
        description:
          `Billed at ${item.unit_price} per unit; the available reference price is ` +
          `${reference.reference_price} (${reference.source_name}). ` +
          `The billed amount is higher than the available reference.`,
        evidence: {
          item: item.name,
