...
export function CompareEstimate({
  ...
  return (
    <div className="rounded-[1rem] border border-success/20 bg-success-bg p-5 sm:p-6">
      ...
      {otherPolicyTerms.length > 0 && <div className="mt-5 rounded-[1rem] border border-warning/25 bg-warning-bg/50 p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.1em] text-warning">Policy context</p><h4 className="mt-1 text-base font-semibold text-text-primary">Other policy terms that may affect the claim</h4></div><StatusBadge tone="warning">Shown separately</StatusBadge></div><div className="mt-4 grid gap-3">{otherPolicyTerms.map((term) => <div key={term.label} className="rounded-xl border border-warning/20 bg-surface p-3"><p className="text-sm font-semibold text-text-primary">{term.label}</p><p className="mt-1 text-xs leading-5 text-text-muted">{term.description}</p></div>)}</div><p className="mt-4 text-xs leading-5 text-text-muted">The estimate below currently calculates the policy sub-limit, deductible, and co-payment rules. These terms are shown separately so the estimate is not mistaken for a final claim decision.</p></div>}
