...
export async function POST() {
  ...
  try {
    const demoCase = await createDemoCase(supabase, userId, "insurance");
    return NextResponse.json({ case: demoCase }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not create the synthetic insurance case";
